...
func TestStickLengths(t *testing.T) {
	tests := []struct {
		name     string
		input    compcoding.StickLengthsStruct
		expected int64
	}{
		{
			name:     "simple",
			input:    compcoding.StickLengthsStruct{ N: 5, Nums: []int64{2, 3, 1, 5, 2} },
			expected: 5,
		},
		{
			name:     "multiple duplicates",
			input:     compcoding.StickLengthsStruct{ N: 5, Nums: []int64{2, 3, 3, 5, 2} },
			expected: 4,
		},
		{
			name:     "single",
		...
		},
{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := compcoding.StickLengths(&tt.input)
			fmt.Printf("Output: %d\n", result)
